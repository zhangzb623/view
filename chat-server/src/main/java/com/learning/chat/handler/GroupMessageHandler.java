...
public class GroupMessageHandler extends SimpleChannelInboundHandler<ChatPacket> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ChatPacket msg) {
        if (msg.getType() != ChatPacketType.GROUP_MESSAGE) {
            ctx.fireChannelRead(msg);
            return;
        }
        if (msg.getFromUserId() == null || msg.getGroupId() == null || !StringUtils.hasText(msg.getContent())) {
            ctx.writeAndFlush(errorResponse(msg, "fromUserId, groupId or content missing"));
            return;
        }
        DeliveryResult result = chatRoutingService.sendGroupMessage(msg);
    ...
    private ChatPacket buildResponse(ChatPacket request, DeliveryResult result) {
        ...
        response.setType(result.isSuccess() ? ChatPacketType.GROUP_MESSAGE : ChatPacketType.ERROR);
