...
@RocketMQMessageListener(
    ...
    consumerGroup = "message-payment-consumer",
    selectorExpression = "*"
)
public class PaymentRocketMQConsumer implements RocketMQListener<PaymentEvent> {
    ...
    public void onMessage(PaymentEvent event) {
        try {
            log.info("接收到支付事件: eventType={}, paymentId={}, userId={}",
            ...
            if ("PAYMENT_CREATED".equals(event.getEventType())) {
                rocketMQProducer.sendOrderDelayMessage(event.getPaymentId(), event.getUserId(), "30s");
            }

        } catch (Exception e) {
            log.error("处理支付事件失败: eventType={}", event.getEventType(), e);
